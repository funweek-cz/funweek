...
export default function OtpLoginPage() {
    ...
    const handleEmailSubmit = async (e) => {
        ...
        if (error) {
            setError(`Chyba při odesílání: ${error.message}`);
        } else {
            setStep(2);
            setCode(['', '', '', '', '', '']);
        }
    ...
    const handleCodeSubmit = async (e) => {
        ...
        if (error) {
            setError(`Chyba ověření: ${error.message}`);
        }
    };
    ...
    if (session) {
        return <div className="min-h-screen flex items-center justify-center">Přesměrování...</div>;
    }
            ...
            <main className="w-full max-w-md bg-white rounded-2xl border overflow-hidden">
                <div className="p-6 sm:p-8">
                    {step === 1 && (
                        <div className="space-y-6">
                            <div className="text-center">
                                <h1 className="text-3xl font-extrabold text-funweek">Vítej zpátky!</h1>
                                <p className="text-funweek/80 mt-2 text-md">
                                    Zadej prosím svůj email, pošleme ti jednorázový kód pro přihlášení.
                                </p>
                            </div>
                            <form onSubmit={handleEmailSubmit} className="space-y-6">
                                <div>
                                    ...
                                    <div className="relative">
                                        <Mail className="absolute left-3 top-1/2 -translate-y-1/2 text-funweek/50" size={20} />
                                        <input
                                            type="email"
                                            value={email}
                    ...
                    {step === 2 && (
                        <div className="space-y-6">
                            <div className="text-center">
                                <h1 className="text-3xl font-extrabold text-funweek">Poslali jsme ti kód!</h1>
                                <p className="text-funweek/80 mt-2 text-md">
                                    Zadej jednorázový kód, který jsme poslali na <span className="font-semibold text-funweek underline">{email}</span>
                                </p>
                            </div>
