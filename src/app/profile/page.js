...
export default function ProfilePage() {
    ...
    const getProfile = useCallback(async () => {
        ...
        let { data, error } = await supabase
            .from('profiles')
            .select(`id, full_name, email, avatar_url, visible_staff, instagram, linkedin, discord, visible_email, thumbnail_image, department, role`)
            .eq('id', user.id)
            .single();
            ...
            <div className="flex items-center gap-5">
                <div className="flex flex-col items-center">
                    ...
                    >
                        ...
                        <div
                            className="absolute inset-0 bg-black text-white bg-opacity-50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                            <Pencil />
                        </div>
                    </div>
