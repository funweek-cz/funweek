...
export default function ProfileEditForm({ initialProfile, onSave }) {
    const departmentParts = initialProfile.department ? initialProfile.department.split(';#') : ['', '#000000'];

    const [thumbnailUrl, setThumbnailUrl] = useState(initialProfile.thumbnail_image || '');
    ...
    const [formData, setFormData] = useState({
        ...
        departmentColor: departmentParts[1] ? `#${departmentParts[1].replace('#', '')}` : '#000000',
    });
    ...
    const inputClass = "w-full p-2 border border-gray-300 rounded mt-1 focus:ring-funweek focus:border-funweek";
        ...
        <div className="mt-10 pt-6 border-t border-gray-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label htmlFor="full_name" className={labelClass}>Tvé jméno</label>
                        <input
                            type="text"
                            id="full_name"
                            name="full_name"
                    ...
                    <div>
                        <label htmlFor="visible_email" className={labelClass}>Kontaktní email (pokud ti byl přiřazen)</label>
                        <input
                            type="email"
                            id="visible_email"
                            name="visible_email"
                    ...
                    <div>
                        <label htmlFor="primary_email" className={labelClass}>Přihlašovací email</label>
                        <input
                            type="email"
                            id="primary_email"
                            name="primary_email"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label htmlFor="discord" className={labelClass}>Discord</label>
                        <input
                            type="text"
                            id="discord"
                            name="discord"
                ...
                {isStaff && (
                    <div className="border border-funweek/50 p-4 rounded-md mt-6 bg-funweek/5">
                        <h3 className="text-xl font-semibold mb-4 text-funweek">Sekce pro dobrovolníky</h3>
                        <div className="mb-6">
                            <label className={labelClass}>Obrázek pro stránku náš tým</label>
                            <div className="text-sm mb-2 font-bold text-red-700">
                                <p>POZOR: Doporučený poměr stran pro thumbnail je 2:3 (šířka:výška). Přečti si <a href="https://www.notion.so/Pravidla-pro-osobn-fotografie-na-u-ivatelsk-m-tu-2b2a3596c03c80b182e9e9852b14dac1?source=copy_link" className="underline">Pravidla pro osobní fotografie na uživatelském účtu</a> pro více detailů.</p>
                            </div>
                            <div className="text-sm text-gray-500 mb-2">
                                <p>Tento obrázek se použije na stránce <a href="/team" className="underline cursor-pointer" target="_blank">náš tým</a>.</p>
                            </div>
                ...
                <button
                    ...
                    className={`w-full py-3 mt-6 text-white font-semibold rounded-md transition-colors ${
                        loading ? 'bg-gray-400 cursor-not-allowed cusror-not-allowed' : 'bg-funweek hover:bg-funweek/80  cursor-pointer'
                    }`}
                    disabled={loading}
                >
